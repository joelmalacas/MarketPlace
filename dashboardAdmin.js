...
function produtos() {
    ...
    for (let i = 0; i < produtos.length; i++) {
        ...
        const imagemSemPrefixo = produto.imagem.replace('/MarketPlace', '');
        // Varivavel para subsituir o caminho da imagem porque dava conflito entre diretorias
        cardImage.src = imagemSemPrefixo;
        ...
        if (CliqueProduto) { // Adicionar evento de clique ao card se for encontrado
            CliqueProduto.addEventListener("click", function() {
                ...
                addToCartButton.addEventListener('click', function() {
                    ...
                    if (cardStatus.textContent === "Esgotado") {
...
function ShopCarrinho() {
    ...
    for (let i = 0; i < Carrinho.length; i++) {
        ...
        const imagemSemPrefixo = Carrinho_produto.Imagem.replace('/MarketPlace', '');
        imageDiv.classList.add('carrinho-card-image');
        imageDiv.src = imagemSemPrefixo;
...
function Pedidos() {
    ...
    if (StoragePedido == 0 || StoragePedido == null) {
        ...
        for (let i = 0; i < StoragePedido.length; i++) {
            ...
            //Criar elemento Imagem
            const imageDiv = document.createElement('img');
            const imagemSemPrefixo = pedido.Imagem.replace('/MarketPlace', '');
            imageDiv.src = imagemSemPrefixo;
            imageDiv.classList.add('carrinho-card-image');
            ...
            nomeEcomenda.innerHTML = "<strong>Nome: </strong>";

            const moradaEcomenda = document.createElement('p');
            moradaEcomenda.innerHTML = "<strong>Morada: </strong>";

            const codPostalEcomenda = document.createElement('p');
            codPostalEcomenda.innerHTML = "<strong>Código Postal: </strong>";

            const StatusEcomenda = document.createElement('p');
            StatusEcomenda.innerHTML = "<strong>Estado: </strong> A preparar Encomenda";
...
function EventoPerfil() {
    ...
    Nome.value = "Administrador";
    Email.value = "Adminsitrador@gmail.com";
    Morada.value = "Rua E-MARKET LISBOA";
    CodPostal.value = "2005-502";
    Localidade.value = "Lisboa";
    NIF.value = "212345678";
    Contacto.value = "+351912345678";
}
